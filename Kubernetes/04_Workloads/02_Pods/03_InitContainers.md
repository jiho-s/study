...
초기화 컨테이너는 파드의 앱 컨테이너들이 실행되기 전에 실행되는 특수한 컨테이너이다. 앱 이미지에는 없는 유틸리티 또는 설정 스크립트 등을 포함할 수 있다.

파드 스펙에 `containers` 와 같이 명시할 수 있다.

### 초기화 컨테이너 이해하기

파드는 다수의 앱 컨테이너를 포함할 수 있고, 또한 앱 컨테이너 실행 전에 동작되는 하나 이상의 초기화 컨테이너도 포함할 수 있다.
...
컨테이너를 초기화 컨테이너로 지정하기 위해서는, 파드 스펙에 앱 `containers` 배열과 나란히 `initContainers` 필드를 컨테이너 타입 오브젝트의 JSON 배열로 추가한다.
...
이 간단한 예제는 사용자만의 초기화 컨테이너를 생성하는데 영감을 줄 것이다. 
